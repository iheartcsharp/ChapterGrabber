...
namespace JarrettVance.ChapterTools
{
    ...
    public static class Grabber
    {
        ...
        private static IEnumerable<string> GoogleTitle(string title)
        {
            ...
            {
                json = web.DownloadString(string.Format("http://ajax.googleapis.com/ajax/services/search/web?v=1.0&q={0}", Uri.EscapeUriString("movie " + title)));
            }
        ...
        public static List<KeyValuePair<int, string>> SuggestTitles(string title)
        {
            ...
            return FilterMovieTitles(GoogleTitle(title))
                .GroupBy(k => k.Value)
                .Select(k => new KeyValuePair<int, string>(k.Sum(t => t.Key), k.Key))
